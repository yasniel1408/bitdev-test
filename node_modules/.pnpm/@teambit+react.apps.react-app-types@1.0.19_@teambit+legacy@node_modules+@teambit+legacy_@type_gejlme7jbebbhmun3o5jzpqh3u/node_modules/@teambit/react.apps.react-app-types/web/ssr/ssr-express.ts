...
export function createExpressSsr({
  name,
  workdir,
  port,
  app,
  ...
  express.use(async (request, response) => {
    ...
    try {
      ...
      logger?.consoleError(
        `[react.application] [ssr] error at "${request.url}"`,
        error
      );
...
export async function loadSsrApp(workdir: string, appName: string) {
  const entryFile = resolve(
    workdir,
    calcOutputPath(appName, 'ssr'),
    MAGIC_FOLDER,
    SSR_ENTRY_FILE
  );
  if (!fs.existsSync(entryFile))
    throw new Error(`expected ssr bundle entry file at "${entryFile}"`);

  const entry = await import(entryFile);
  const app = entry?.default;
  if (!app)
    throw new Error(
      `bundle entry file has no default export (at "${entryFile}")`
    );
...
export function parseAssets(
  ...
  if (deadAssets.length > 0)
    throw new Error(
      'missing some build assets (maybe need to turn on cachedAssets, etc)'
    );

...
const ReactEnv = (context) => {
    const { name, namePascalCase: Name, envId } = context;
    const envVersion = envId
        ? `/**
 * this env extends react-env version ${envId.version}.
 * to inspect its config @see https://bit.cloud/teambit/react/react-env?version=${envId.version}
 * */
`
        : '';
    return {
        relativePath: `${name}.bit-env.ts`,
        content: (0, dedent_1.default) `${envVersion}import { ReactEnv } from '@teambit/react.react-env';

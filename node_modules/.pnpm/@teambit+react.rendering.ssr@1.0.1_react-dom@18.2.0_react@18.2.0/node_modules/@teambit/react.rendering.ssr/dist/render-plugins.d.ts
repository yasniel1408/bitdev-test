...
export declare type ContextProps<T = unknown> = {
    renderCtx?: T;
    children: ReactNode;
};
export declare type ServerRenderPlugin<RenderCtx = unknown> = {
...
export declare type ClientRenderPlugin<RenderCtx = unknown, Deserialized = unknown> = {
    /** identifies plugin data during serialization / deserialization */
    key?: string;
    /**
     ...
     * Context state will only be available to the current Aspect, in the other hooks, as well as a prop to the react context component.
     */
    browserInit?: (deserializedData: Deserialized) => RenderCtx | void | undefined | Promise<RenderCtx | void | undefined>;
    /**
...
export interface RenderPlugin<RenderCtx = unknown, Deserialized = unknown> extends ServerRenderPlugin<RenderCtx>, ClientRenderPlugin<RenderCtx, Deserialized> {
    /**
     * Unified React Decorator. Will receive render context, produced by `onBeforeRender()` (server-side) or `deserialize()` (browser)
     */
    reactContext?: ComponentType<ContextProps<RenderCtx>>;
